Moxycart.grid.ContainerMoxycart = function(config) {
    config = config || {};
    ...
    Ext.applyIf(config,{
        id: 'moxycart-grid-container-moxycart'
        ,title: _('moxycart.moxycart')
        ...
        ,sm: this.sm
        ,plugins: [this.exp]
        ,emptyText: _('moxycart.moxycart_none')
        ,columns: [this.sm,this.exp,{
            ...
            header: _('moxycart.article_title')
            ,dataIndex: 'pagetitle'
			,id: 'main'
            ,width: 200
            ,sortable: true
            ...
            header: _('moxycart.author')
            ,dataIndex: 'createdby_username'
            ,width: 150
            ,sortable: true
            ,renderer: {fn:this._renderAuthor,scope:this}
        },{
            header: '<img src="'+Moxycart.assets_url+'images/comments-icon-w.png" alt="" class="moxycart-comments-col-header" />'
            ,dataIndex: 'comments'
            ,width: 50
            ,sortable: true
            ,hidden: !Moxycart.commentsEnabled
            ,renderer: {fn:this._renderComments,scope:this}
        },{
            header: _('moxycart.tags')
            ,dataIndex: 'tags'
            ,width: 200
            ,sortable: true
        }]
        ,tbar: [{
            text: _('moxycart.article_create')
            ,handler: this.createArticle
            ,scope: this
        },{
            text: _('bulk_actions')
            ,menu: [{
                text: _('moxycart.article_selected_publish')
                ,handler: this.publishSelected
                ,scope: this
            },{
                text: _('moxycart.article_selected_unpublish')
                ,handler: this.unpublishSelected
                ,scope: this
            },'-',{
                text: _('moxycart.article_selected_delete')
                ,handler: this.deleteSelected
                ,scope: this
            },{
                text: _('moxycart.article_selected_undelete')
                ,handler: this.undeleteSelected
                ,scope: this
            }]
        },'-',{
            text: _('moxycart.moxycart_import')
            ,handler: this.importMoxycart
            ,scope: this

        },'->',{
            xtype: 'moxycart-combo-filter-status'
            ,id: 'moxycart-grid-filter-status'
            ,value: ''
            ,listeners: {
    ...
    Moxycart.grid.ContainerMoxycart.superclass.constructor.call(this,config);
    this._makeTemplates();
    this.on('rowclick',MODx.fireResourceFormChange);
    this.on('click', this.onClick, this);
};
Ext.extend(Moxycart.grid.ContainerMoxycart,MODx.grid.Grid,{
    getMenu: function() {
        ...
        m.push({
           text: _('moxycart.article_edit')
           ,handler: this.editArticle
        });
        m.push({
           text: _('moxycart.article_duplicate')
    ...
    ,clearFilter: function() {
        ...
        Ext.getCmp('moxycart-article-search').reset();
    	this.getBottomToolbar().changePage(1);
        this.refresh();
    }
    ...
    ,_makeTemplates: function() {
        this.tplPublished = new Ext.XTemplate('<tpl for=".">'
            +'<div class="moxycart-grid-date">{publishedon_date}, {publishedon_year}<span class="moxycart-grid-time">{publishedon_time}</span></div>'
        +'</tpl>',{
			compiled: true
		});
        this.tplComments = new Ext.XTemplate('<tpl for=".">'
            +'<div class="moxycart-grid-comments"><span>{comments}</span></div>'
        +'</tpl>',{
			compiled: true
		});
    ...
    ,createArticle: function(btn,e) {
        ...
        if (panel && panel.record) {
            tpl = '&template=' + panel.record.moxycart_container_settings.articleTemplate
        }
    ...
    ,importMoxycart: function(btn,e) {
        var r = {
            'id': MODx.request.id
            ,'service': 'MODX'
        };
        if (!this.windows.importMoxycart) {
            this.windows.importMoxycart = MODx.load({
                xtype: 'moxycart-window-import'
                ,record: r
                ,listeners: {
        ...
        this.windows.importMoxycart.setValues(r);
        this.windows.importMoxycart.show(e.target);
    }

    ,deleteArticle: function(btn,e) {
        MODx.msg.confirm({
            title: _('moxycart.article_delete')
            ,text: _('moxycart.article_delete_confirm')
    ...
    ,deleteSelected: function(btn,e) {
        ...
        MODx.msg.confirm({
            title: _('moxycart.article_delete_multiple')
            ,text: _('moxycart.article_delete_multiple_confirm')
            ,url: this.config.url
            ,params: {
...
Ext.reg('moxycart-grid-container-moxycart',Moxycart.grid.ContainerMoxycart);
