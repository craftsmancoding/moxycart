...
class Field extends BaseModel
{
    ...
    public function generate($field_id, $value = '', $name = null)
    {
        if (!$F = $this->modx->getObject('Field', $field_id)) {
            $this->modx->log(\modX::LOG_LEVEL_ERROR, 'Field id not found: ' . $field_id, '', __CLASS__, __FUNCTION__,
                __LINE__);
            return false;
        }
        $type = $F->get('type');
        ...
        switch ($type) {
            case 'dropdown':
                $out = \Formbuilder\Form::dropdown($name, $args, $value,
                    array('label' => $attr['label'], 'description' => $attr['description']));
            ...
            case 'asset':
                ...
                break;
            default:
                $this->modx->log(\modX::LOG_LEVEL_ERROR, 'Unsupported field type: ' . $type, '', __CLASS__,
    ...
    public function asset($name, $asset_id = '', $args = array(), $tpl = null)
    {
        ...
        if ($A = $this->modx->getObject('Asset', $asset_id)) {
            $url = $A->getThumbnailURL($this->modx->getOption('moxycart.thumbnail_width'),
                $this->modx->getOption('moxycart.thumbnail_height'));
            $img = sprintf('<img src="%s" width="%s" height="%s"/>', $url,
                $this->modx->getOption('moxycart.thumbnail_width'),
                $this->modx->getOption('moxycart.thumbnail_height'));
        }
